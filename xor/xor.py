...
dge/dt=-ge/taue : 1
'''
spiketimes = [(0, 3 * ms), (0, 4 * ms),(1, 2 * ms), (1, 4 * ms)]
input = SpikeGeneratorGroup(2, spiketimes)
...
dA_post *= gmax
dA_pre *= gmax
stdp = STDP(synapses, eqs=eqs_stdp, pre='A_pre+=dA_pre;w+=A_post', post='A_post+=dA_post;w+=A_pre', wmax=gmax)
